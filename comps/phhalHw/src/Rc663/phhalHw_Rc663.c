    ...
    if (status == PH_ERR_SUCCESS)
    {
        ...
        do
        {
            do
            {
                ...
                PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_ReadRegister(pDataParams, PHHAL_HW_RC663_REG_STATUS, &bStatus2));
            }
    ...
    )
{
    ...
    case PHHAL_HW_CARDTYPE_ISO14443A:
        if((wDerivate == PHHAL_HW_CLRC663_PRODUCT_ID) || (wDerivate == PHHAL_HW_MFRC631_PRODUCT_ID) 
    ...
    case PHHAL_HW_CARDTYPE_FELICA:
        if(wDerivate == PHHAL_HW_CLRC663_PRODUCT_ID)
        {
            /* Use Felica shadow */
    ...
    case PHHAL_HW_CARDTYPE_ISO15693:
        if((wDerivate == PHHAL_HW_CLRC663_PRODUCT_ID) || (wDerivate == PHHAL_HW_SLRC610_PRODUCT_ID))
        {
            /* Use ISO15693 shadow */
                                   ...
                                   )
{
    ...
    case PHHAL_HW_CONFIG_FRAME_SYNC:
        ...
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_WriteRegister(pDataParams, PHHAL_HW_RC663_REG_FRAMECON, 0x02));
        ...
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_WriteRegister(pDataParams, PHHAL_HW_RC663_REG_TXCRCCON, 0x7B));
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_WriteRegister(pDataParams, PHHAL_HW_RC663_REG_RXCRCCON, 0x7B));
        break;

    case PHHAL_HW_CONFIG_TXDATANUM:

        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_WriteRegister(pDataParams, PHHAL_HW_RC663_REG_TXDATANUM, (uint8_t)wValue));
        break;
...
phStatus_t phhalHw_Rc663_ExecuteCmd(
                                    phhalHw_Rc663_DataParams_t * pDataParams,
                                    uint8_t bCmd,  
                                    uint16_t wOption,
